...
想象一下你在一家汽车工厂工作。工厂不会每次都从零开始造车，而是：
1. 先设计一个**图纸/蓝图**（定义车应该有什么：4个轮子、方向盘、发动机等）
2. 根据蓝图**批量生产**汽车（每辆车都按照图纸制造，但颜色、配置可以不同）
3. 每辆车都有**自己的特点**（有的是红色2021年款，有的是黑色2023年款）
4. 每辆车都有**共同的功能**（都能开、都能鸣笛、都能刹车）

**面向对象编程（Object-Oriented Programming，OOP）** 就是用这种思维写代码：

- **类（Class）** = 汽车图纸（蓝图/模板）
- **对象（Object）** = 具体的汽车（根据图纸生产出来的实体）
- **属性（Attribute）** = 汽车的特征（颜色、年份、油量）
- **方法（Method）** = 汽车的功能（加速、刹车、鸣笛）
...
# 计算平均分？需要写100个变量名！
average = (student1_score + student2_score + ... + student100_score) / 100

# 修改学生信息？需要找到正确的变量
student1_score = 90  # 这是哪个学生？看变量名才知道
```
...
# 修改学生信息 - 直观明了
students[0].score = 90  # 第一个学生的分数
students[0].study()     # 让第一个学生学习
```
...
不管是金毛、哈士奇还是泰迪，都可以用"狗"这个抽象概念来描述。

**3. 继承（Inheritance）**
子类可以继承父类的属性和方法，实现代码复用。（下一章详细学习）

**4. 多态（Polymorphism）**
同一个方法在不同对象中有不同的实现。（下一章详细学习）
...
# 对象 = 具体的汽车
my_car = Car("Tesla", "白色", 2023)
your_car = Car("BMW", "黑色", 2022)

# 每辆车独立工作
my_car.start()           # Tesla 启动了！
my_car.accelerate(50)    # 当前速度: 50 km/h
your_car.accelerate(80)  # 当前速度: 80 km/h（不影响my_car）
```
...
# 每个账户相互独立
zhang_account = BankAccount("张三", 1000)
li_account = BankAccount("李四", 500)

zhang_account.deposit(500)   # 张三存款，不影响李四
zhang_account.withdraw(200)  # 张三取款
li_account.withdraw(100)     # 李四取款
```
...
**面向对象编程（OOP）** 是一种编程范式，把相关的数据和功能组织成"对象"。这不仅让代码更有组织性，还让程序更容易理解、维护和扩展。
